...
export default function GlobeBackground({ containerId = 'threejs-container' }) {
  ...
  useEffect(() => {
    ...
    function resetMeteor(line, init = false) {
      line.position.set((Math.random() - 0.35) * 32 + 5, Math.random() * 16 + 5, -10 - Math.random() * 10)
      line.scale.setScalar(0.7 + Math.random() * 0.8)
    ...
    const moonGeo = new THREE.SphereGeometry(0.72, 24, 18)  // was 32×24
    ...
    function animate(now) {
      ...
      moonMesh.position.set(
        ...
        Math.sin(moonAngle) * 2.1 + Math.sin(moonAngle * 0.5) * 0.4,
